...
def build_session_scoped_pipe_security_attributes(session_id: int, logger=None):
    ...
    try:
        ...
        try:
            ...
            # pywin32 版本差异：可能是 (PSID, attributes) 元组、带 .Sid 的包裹对象、
            # 或直接是 PySID（frozen 1.6.1 实测）——三种形态统一取 SID 对象
            raw_user = token_user[0] if isinstance(token_user, tuple) else token_user
            user_sid = getattr(raw_user, "Sid", raw_user)
...
def _kernel32():
    """kernel32 绑定（ctypes 直连，不依赖 pywin32 封装版本）。"""
    import ctypes
    import ctypes.wintypes

    k32 = ctypes.windll.kernel32
    k32.GetNamedPipeClientSessionId.argtypes = [ctypes.wintypes.HANDLE, ctypes.POINTER(ctypes.wintypes.ULONG)]
    k32.GetNamedPipeClientSessionId.restype = ctypes.wintypes.BOOL
    ...
    k32.ProcessIdToSessionId.argtypes = [ctypes.wintypes.DWORD, ctypes.POINTER(ctypes.wintypes.DWORD)]
    k32.ProcessIdToSessionId.restype = ctypes.wintypes.BOOL
...
def _handle_int(pipe_handle) -> int:
    try:
        return int(pipe_handle)
    except (TypeError, ValueError):
        return 0


def get_pipe_client_session_id(pipe_handle) -> int | None:
    """获取管道连接的客户端会话 ID（P0-5/P0-03）。

    pywin32 封装优先，失败/返回 0 时回退 ctypes kernel32；
    均不可得返回 None（调用方走二次校验或 fail-closed）。
    """
    try:
        import win32pipe as _wp

        value = int(_wp.GetNamedPipeClientSessionId(pipe_handle))
        if value > 0:
            return value
    except Exception:
        pass
    try:
        k32 = _kernel32()
        out = ctypes.wintypes.ULONG()
        if k32.GetNamedPipeClientSessionId(_handle_int(pipe_handle), ctypes.byref(out)):
            value = int(out.value)
...
def get_pipe_client_process_id(pipe_handle) -> int | None:
    """获取管道连接的客户端 PID（pywin32 优先，ctypes kernel32 兜底）。"""
    try:
        import win32pipe as _wp

        pid = int(_wp.GetNamedPipeClientProcessId(pipe_handle))
        if pid > 0:
            return pid
    except Exception:
        pass
    try:
        k32 = _kernel32()
        out = ctypes.wintypes.ULONG()
        if k32.GetNamedPipeClientProcessId(_handle_int(pipe_handle), ctypes.byref(out)):
            pid = int(out.value)
...
def get_process_session_id(pid: int) -> int | None:
    """ProcessIdToSessionId（ctypes，不依赖 pywin32 版本）。"""
    try:
        k32 = _kernel32()
        session_id = ctypes.wintypes.DWORD()
        if k32.ProcessIdToSessionId(int(pid), ctypes.byref(session_id)):
...
def get_pipe_client_process_session_id(pipe_handle) -> int | None:
    """二次校验路径（P0-03）：客户端 PID → ProcessIdToSessionId。

    GetNamedPipeClientSessionId 在部分环境返回 0（pywin32 已知问题），
    此时通过客户端进程 ID 反查会话 ID；仍不可得则返回 None（调用方 fail-closed）。
    """
    pid = get_pipe_client_process_id(pipe_handle)
    if not pid:
        return None
    return get_process_session_id(pid)


def get_process_image_path(pid: int) -> str | None:
    """读取进程映像完整路径（QueryFullProcessImageName，权限不足返回 None）。"""
    try:
        import ctypes
        import ctypes.wintypes
...
class NamedPipeCommandServer:
    ...
    def _serve_forever(self) -> None:
        ...
        if self.expected_session_id is not None:
            ...
            if security_attributes is None:
                # P0-03 fail-closed：会话范围 DACL 构建失败时不再回退 world-readable，
                # 使用默认受限 DACL（SYSTEM/Administrators），配合连接级 fail-closed 会话校验。
                # 可用性影响：若 WTSQueryUserToken 不可用导致 DACL 缺失，用户态客户端可能被拒。
                self.logger(
                    "session-scoped pipe DACL unavailable; "
                    "using default restrictive DACL + fail-closed session verification (P0-03)"
                )
                security_attributes = None
    ...
    def _is_trusted_client_process(self, pipe_handle) -> bool:
        """会话不可解析时的最后回退（P0-03）：客户端映像路径与当前 Agent 二进制一致。

        同一 Agent 安装目录下的自身进程（UserSessionAgent/Helper 互连）才可信；
        冒充需要对该目录的写权限（默认仅管理员）。
        """
        import os
        import sys
        ...
        if trusted:
            self.logger(
                f"named pipe client trusted by same-binary image path: pid={pid} image={image_path}"
            )
        return trusted

    def _handle_client(self, pipe_handle) -> None:
        """在单个管道连接上循环处理多轮请求-响应（长连接模式）。"""
        # P0-03 fail-closed：客户端会话校验——主路径（GetNamedPipeClientSessionId）
        # 不可用（返回 0/None，pywin32 已知问题）时，走二次校验（客户端 PID →
        # ProcessIdToSessionId）；二次校验仍不可得则直接拒绝连接，不再 fail-open。
        if self.enforce_session_scope and self.expected_session_id is not None:
            expected = int(self.expected_session_id)
            verified_via = None
            reject_reason = None
            client_sid = get_pipe_client_session_id(pipe_handle)
            if client_sid and int(client_sid) > 0:
                if int(client_sid) == expected:
                    verified_via = "pipe_session_id"
                else:
                    reject_reason = f"session mismatch client={client_sid} expected={expected}"
            else:
                proc_sid = get_pipe_client_process_session_id(pipe_handle)
                if proc_sid is not None and int(proc_sid) > 0:
                    if int(proc_sid) == expected:
                        verified_via = "process_session_id"
                    else:
                        reject_reason = (
                            f"session mismatch (via client process) client={proc_sid} expected={expected}"
                        )
                else:
                    # P0-03 最后回退：会话完全不可解析时仅信任与自身同二进制的客户端
                    if self._is_trusted_client_process(pipe_handle):
                        verified_via = "trusted_same_binary"
                    else:
                        reject_reason = (
                            f"session unverifiable (pipe_session_id={client_sid}, "
                            f"process session unavailable, image not trusted) — fail-closed (P0-03)"
                        )
            if verified_via is None:
                self.logger(f"named pipe client rejected: {reject_reason}")
                try:
                    ...
                    pass
                return
            if verified_via == "process_session_id":
                self.logger(
                    f"named pipe client allowed via process-session fallback: session={expected}"
                )
        # R4：自定义客户端校验（返回 False 即拒绝；P0-03：校验异常按拒绝处理，不再 fail-open）
        if self.client_validator is not None:
            client_sid_v = get_pipe_client_session_id(pipe_handle)
            if client_sid_v is None or int(client_sid_v) <= 0:
                client_sid_v = get_pipe_client_process_session_id(pipe_handle) or 0
            if int(client_sid_v) <= 0:
                # P0-03：会话不可解析时仅信任与自身同二进制的客户端，否则拒绝
                if self._is_trusted_client_process(pipe_handle):
                    pass  # 同 Agent 二进制互连，跳过会话白名单校验
                else:
                    self.logger(
                        "named pipe client rejected: session unverifiable and image not trusted (P0-03)"
                    )
                    try:
                        win32pipe.DisconnectNamedPipe(pipe_handle)
                        win32file.CloseHandle(pipe_handle)
                ...
                try:
                    ...
                    self.logger(f"named pipe client validator error (fail-closed): {exc}")
                    try:
                        win32pipe.DisconnectNamedPipe(pipe_handle)
                        win32file.CloseHandle(pipe_handle)
                    except Exception:
