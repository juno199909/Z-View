...
call :log_info "deploy completed successfully"
set "EXIT_CODE=0"
if "%SILENT_MODE%"=="0" (
    color 0A
    echo.
...
if errorlevel 1 exit /b 1
call :restart_service_post_deploy
if errorlevel 1 exit /b 1
call :wait_for_backend_worker_ready
if errorlevel 1 (
    call :log_error "backend worker --run-agent --no-remote-desktop not detected after post-deploy restart"
    exit /b 1
)
call :log_info "backend worker detected after post-deploy restart"
call :log_backend_worker_snapshot
call :wait_for_user_session_agent_if_interactive
if errorlevel 1 exit /b 1
call :verify_remote_desktop_port_owner_if_interactive
if errorlevel 1 exit /b 1
call :verify_console_host_session_if_interactive
if errorlevel 1 exit /b 1
exit /b 0

:restart_service_post_deploy
call :log_info "restarting Windows service after deploy for console-host stabilization"
sc stop "%SERVICE_NAME%" >nul 2>&1
call :wait_for_service_state "Stopped" 35
if errorlevel 1 (
    call :log_error "service did not stop during post-deploy restart"
    exit /b 1
)
call :log_info "service stopped for post-deploy restart"
sc start "%SERVICE_NAME%" >nul 2>&1
call :wait_for_service_state "Running" 35
if errorlevel 1 (
    call :log_error "service did not return to Running during post-deploy restart"
    exit /b 1
)
call :log_info "service restarted successfully after deploy"
exit /b 0
...
:verify_console_host_session_if_interactive
call :log_console_host_snapshot
powershell -NoProfile -ExecutionPolicy Bypass -Command ^
    "& {" ^
    "  Add-Type -Namespace Win32 -Name Native -MemberDefinition '[System.Runtime.InteropServices.DllImport(\"kernel32.dll\")] public static extern uint WTSGetActiveConsoleSessionId();' -ErrorAction SilentlyContinue | Out-Null;" ^
    "  function Get-ActiveConsoleSessionIdSafe { try { return [int][Win32.Native]::WTSGetActiveConsoleSessionId() } catch { return -1 } }" ^
    "  function Get-SessionName([int]$sessionId) {" ^
    "    $lines = @(query session 2^>^&1);" ^
    "    foreach ($line in $lines) {" ^
    "      $clean = ($line -replace '^>','').Trim();" ^
    "      if (-not $clean) { continue };" ^
    "      if ($clean -match '^(?<name>\\S+)\\s+.*?\\s+(?<id>\\d+)\\s+(?<state>Active|Disc|Conn|Listen)\\b') { if ([int]$matches['id'] -eq $sessionId) { return $matches['name'] } }" ^
    "    }" ^
    "    return ''" ^
    "  }" ^
    "  $explorerSessions = @(Get-WmiObject Win32_Process -Filter \"Name='explorer.exe'\" -ErrorAction SilentlyContinue | Select-Object -ExpandProperty SessionId -Unique | Where-Object { $_ -gt 0 });" ^
    "  $activeConsoleSession = Get-ActiveConsoleSessionIdSafe;" ^
    "  if ($explorerSessions.Count -eq 0 -and $activeConsoleSession -le 0) { exit 2 }" ^
    "  $listeners = @(Get-NetTCPConnection -LocalPort 9000 -State Listen -ErrorAction SilentlyContinue);" ^
    "  if ($listeners.Count -eq 0) { exit 1 }" ^
    "  foreach ($listener in $listeners) {" ^
    "    $proc = Get-WmiObject Win32_Process -Filter ('ProcessId=' + $listener.OwningProcess) -ErrorAction SilentlyContinue;" ^
    ...
    "    $cmd = (($proc.CommandLine -replace '\s+', ' ').Trim());" ^
    "    $isValidUserAgent = $proc.Name -eq '%EXE_NAME%' -and $cmd -like '*--user-session-agent*' -and $cmd -notlike '*--service-host*' -and $cmd -notlike '*--run-agent*' -and $cmd -notlike '*--consent-ui*';" ^
    "    if (-not $isValidUserAgent) { continue };" ^
    "    $ownerSession = [int]$proc.SessionId;" ^
    "    $station = Get-SessionName $ownerSession;" ^
    "    if ($activeConsoleSession -gt 0 -and $ownerSession -eq $activeConsoleSession -and $station -ieq 'console') { exit 0 };" ^
    "    if ($activeConsoleSession -gt 0 -and $ownerSession -eq $activeConsoleSession) { exit 3 };" ^
    "    exit 1;" ^
    "  }" ^
    "  exit 1" ^
    "}"
set "CONSOLE_VERIFY_RESULT=%errorLevel%"
if "%CONSOLE_VERIFY_RESULT%"=="0" (
    call :log_info "console host session verified"
    exit /b 0
)
if "%CONSOLE_VERIFY_RESULT%"=="2" (
    call :log_info "no interactive desktop detected; console host verification skipped"
    exit /b 0
)
if "%CONSOLE_VERIFY_RESULT%"=="3" (
    call :log_warn "user-session agent matches active console session, but session name is not confirmed as console yet"
    call :log_runtime_log_tail
    exit /b 0
)
call :log_error "user-session agent has not switched to console host session"
call :log_user_session_agent_snapshot
call :log_runtime_log_tail
exit /b 1
...
for /f "usebackq delims=" %%L in (`powershell -NoProfile -ExecutionPolicy Bypass -Command ^
    ...
    "  }" ^
    "  $lines = @();" ^
    "  $activeConsole = Get-ActiveConsoleSessionIdSafe;" ^
    "  $lines += ('console-host verification: active_console_session=' + $activeConsole);" ^
    "  foreach ($explorer in @(Get-WmiObject Win32_Process -Filter \"Name='explorer.exe'\" -ErrorAction SilentlyContinue | Sort-Object SessionId, ProcessId)) {" ^
    "    $lines += ('console-host verification: explorer pid=' + $explorer.ProcessId + ' session=' + $explorer.SessionId + ' station=' + (Get-SessionName ([int]$explorer.SessionId)))" ^
    "  };" ^
    "  foreach ($listener in @(Get-NetTCPConnection -LocalPort 9000 -State Listen -ErrorAction SilentlyContinue)) {" ^
    "    $proc = Get-WmiObject Win32_Process -Filter ('ProcessId=' + $listener.OwningProcess) -ErrorAction SilentlyContinue;" ^
    "    if ($proc) {" ^
    "      $lines += ('console-host verification: port=9000 pid=' + $proc.ProcessId + ' session=' + $proc.SessionId + ' station=' + (Get-SessionName ([int]$proc.SessionId)) + ' name=' + $proc.Name + ' cmd=' + (($proc.CommandLine -replace '\s+', ' ').Trim()))" ^
    "    }" ^
    "  };" ^
    "  $lines | Select-Object -Unique" ^
    "}"`) do call :log_info "%%L"
...
for /l %%I in (1,1,5) do (
    powershell -NoProfile -ExecutionPolicy Bypass -Command ^
        ...
        "    if ((Get-FileHash -LiteralPath $src -Algorithm SHA256).Hash -ne (Get-FileHash -LiteralPath $dst -Algorithm SHA256).Hash) { throw 'destination hash mismatch after replace'; }" ^
        "    if (Test-Path -LiteralPath $backup) { Remove-Item -LiteralPath $backup -Force -ErrorAction SilentlyContinue }" ^
        "    exit 0" ^
        "  } catch {" ^
    ...
    if exist "%COPY_ERROR_LOG%" (
        set /p COPY_ERROR_MESSAGE=<"%COPY_ERROR_LOG%"
        if defined COPY_ERROR_MESSAGE call :log_warn "copy detail: !COPY_ERROR_MESSAGE!"
        set "COPY_ERROR_MESSAGE="
    )
    timeout /t 2 /nobreak >nul
)
