...
Copy-Item -LiteralPath $ExePath -Destination (Join-Path $PackageDir "Z-View.exe") -Force
Copy-Item -LiteralPath (Join-Path $ProjectRoot "config.json") -Destination (Join-Path $PackageDir "config.json") -Force
...
if (Test-Path $DriverSourceDir) {
    ...
    Copy-Item -LiteralPath $DriverSourceDir -Destination $PackageDir -Recurse -Force
}

# 中文注释：复制完成后立即做静态验收，避免把不完整或损坏的部署包交付给 GPO。
Write-Host "==> Verify release package"
& $VerifyScript -BuildExePath $ExePath
