...
def _load_legacy_module():
    for candidate in _candidate_legacy_paths():
        ...
        return module
    raise RuntimeError("无法加载历史授权托盘模块：cmdb_agent_consent_ui.pyc 不存在")
...
def _determine_dialog_backends(self) -> list[str]:
    major_version, _minor_version = _get_windows_version()
    backends: list[str] = []

    if getattr(self, "_task_dialog_indirect", None):
        backends.append("taskdialog")

    # Tkinter keeps the live countdown experience for systems where the native
    # task dialog is unavailable or blocked by shell/session policies.
    backends.append("tkinter")

    # Win7/locked-down desktops should always retain a final messagebox path.
    if major_version <= 6 and "messagebox" not in backends:
        backends.append("messagebox")
    elif "messagebox" not in backends:
        backends.append("messagebox")
...
def _invoke_consent_prompt(self, title: str, requester: str, origin: str, target: str, timeout_seconds: int, style: int) -> int:
    ...
    for backend in _determine_dialog_backends(self):
        try:
...
def _invoke_tk_consent_dialog(self, title: str, requester: str, origin: str, target: str, timeout_seconds: int) -> int:
    ...
    header = ttk.Label(container, text="Z-View 远程控制确认", font=("Microsoft YaHei UI", 12, "bold"))
    ...
    allow_button = ttk.Button(button_row, text="允许", command=lambda: _finish(IDYES))
    allow_button.grid(row=0, column=0, padx=(0, 8))

    reject_button = ttk.Button(button_row, text="拒绝", command=lambda: _finish(IDNO))
...
def _patched_show_consent_dialog(self, request: dict) -> tuple[bool, str]:
    requester = str(request.get("requester") or "未知管理员")
