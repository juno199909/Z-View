...
_BACKUP_PATH = Path(__file__).with_suffix(".py.backup")
...
def load_core_module():
    loader = SourceFileLoader("cmdb_agent_unified_v2_backup", str(_BACKUP_PATH))
    spec = spec_from_loader(loader.name, loader)

    if spec is None:
        raise ImportError(f"无法为 {_BACKUP_PATH} 创建模块加载规范")
