...
def resolve_config_path() -> Path:
    candidates = [
        APP_BASE_DIR / "config.local.json",
        APP_BASE_DIR / "config.json",
...
def build_consent_authkey(config: dict[str, Any] | None = None) -> bytes:
    ...
    token = str(source.get("token") or os.environ.get("ZVIEW_AGENT_TOKEN") or "")
...
def build_high_integrity_helper_launch_command(
    session_id: int | None = None,
) -> list[str]:
    args = ["--high-integrity-helper"]
    if session_id is not None:
        args.extend(["--target-session-id", str(int(session_id))])
    return _build_agent_command(args)
