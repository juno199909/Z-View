...
def agent_heartbeat(data: dict, request: Request):
    ...
    try:
        ...
        if agent_auth_type == "device":
            ...
            try:
                heartbeat_credential = _issue_agent_device_credential(asset_id, cursor)
                conn.commit()
...
def _issue_agent_device_credential(asset_id: int, cursor) -> Optional[Dict[str, Any]]:
    """为资产签发设备凭据（幂等：已有 active 凭据时不重发，明文无法恢复）。
    返回 {"agent_id":..., "device_secret":...} 或 None（已注册）。"""
    cursor.execute(
...
def list_agent_credentials(request: Request):
    """设备凭据注册状态（含未注册资产）"""
    require_request_permission(request, resolve_required_permission(request.url.path, request.method))
    conn = get_db_connection()
    if not conn:
        raise HTTPException(status_code=500, detail="Database connection failed")
    cursor = conn.cursor(dictionary=True)
