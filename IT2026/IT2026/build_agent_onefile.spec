# -*- coding: utf-8 -*-
# V1.5/V1.6 过渡期兼容构建：onefile 单文件包。
# 用途：让存量 1.8.2 终端先升到携带迁移能力与升级护栏的 1.8.3+，
# 再接收 onedir-zip 完成向新布局（versions/current + ZViewUpdater）迁移。
# 新布局日常构建请使用 build_agent.spec（onedir）。
