<template>
  <div class="zv-panel">
    <div class="zv-panel-header">
      <div>
        <h3 class="zv-panel-title">资产档案</h3>
...
import {
  ...
  InfoFilled, Box, Connection, Share, Cpu
...
const loadAssetHistory = async () => {
  ...
  try {
    const [statusData, historyData, changesData, uptimeData] = await Promise.all([
      getAssetStatus(props.assetId),
      getAssetStatusHistory(props.assetId, { limit: 20 }),
      getAssetChanges(props.assetId, { page: 1, page_size: 20 }),
      getAssetUptime(props.assetId, { days: 7 })
    ])
...
const loadDetail = async () => {
  ...
  try {
    const data = await getAssetDetail(props.assetId)
...
onMounted(() => {
  loadDetail()
  if (props.autoEdit) editMode.value = true
})
