...
let pendingRemoteDesktopOpen = null
...
const scheduleRemoteDesktopUnmount = () => {
  ...
  remoteDesktopUnmountTimer = setTimeout(() => {
    if (!showRemoteDesktop.value && !pendingRemoteDesktopOpen) {
      console.info('[remote-desktop][overview] force unmount fallback')
...
const mountRemoteDesktop = () => {
  console.info('[remote-desktop][overview] mount instance:', currentAssetId.value, currentIpAddress.value)
...
const handleRemoteDesktopVisibleChange = (visible) => {
  console.info('[remote-desktop][overview] visible change:', visible)
...
const handleRemoteDesktopClosed = () => {
  console.info('[remote-desktop][overview] closed event')
  clearRemoteDesktopUnmountTimer()
  showRemoteDesktop.value = false
  const nextOpen = pendingRemoteDesktopOpen
  pendingRemoteDesktopOpen = null
  if (nextOpen) {
    currentAssetId.value = nextOpen.id
    currentIpAddress.value = nextOpen.ipAddress
    currentHostname.value = nextOpen.hostname
    mountRemoteDesktop()
    return
  }
  remoteDesktopMounted.value = false
...
const openRemoteDesktop = (row) => {
  console.info('[remote-desktop][overview] open request:', row?.id, row?.ip_address)
  clearRemoteDesktopUnmountTimer()
  pendingRemoteDesktopOpen = {
    id: row.id,
  ...
  if (remoteDesktopMounted.value || showRemoteDesktop.value) {
    console.info('[remote-desktop][overview] queue reopen after close:', pendingRemoteDesktopOpen.id, pendingRemoteDesktopOpen.ipAddress)
    showRemoteDesktop.value = false
    return
  }

  currentAssetId.value = pendingRemoteDesktopOpen.id
  currentIpAddress.value = pendingRemoteDesktopOpen.ipAddress
  currentHostname.value = pendingRemoteDesktopOpen.hostname
  pendingRemoteDesktopOpen = null
  mountRemoteDesktop()
}
