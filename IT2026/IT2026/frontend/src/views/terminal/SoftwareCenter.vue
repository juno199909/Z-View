<template>
  <div class="zv-page">
    ...
    <el-tabs v-model="activeTab" class="zv-tabs">
      ...
      <!-- 黑白名单（单策略多规则，避免污染策略空间） -->
      <el-tab-pane label="黑白名单" name="policy">
        <el-alert type="info" :closable="false" show-icon class="zv-bw-tip"
          title="检测说明"
          description="Agent 定期比对终端软件清单，发现违规安装将记录审计并产生告警（事后检测）。" />
        <div class="zv-policy-grid">
          <div class="zv-card zv-card-pad">
            ...
            <div class="zv-policy-add">
              <el-input v-model="newWhiteName" placeholder="软件名" size="small" style="width: 260px" @keyup.enter="addPolicy('white')" />
          ...
          <div class="zv-card zv-card-pad">
            ...
            <div class="zv-policy-add">
              <el-input v-model="newBlackName" placeholder="软件名" size="small" style="width: 260px" @keyup.enter="addPolicy('black')" />
...
// 黑白名单：每类一个策略（多规则），记录策略 id 与当前规则集
const bwPolicy = reactive({ whitelist: null, blacklist: null })
const bwRules = reactive({ whitelist: [], blacklist: [] })
...
const loadPolicies = async () => {
  ...
  try {
    // 每类名单只维护一个策略（多规则），避免一软件一策略污染策略空间
    const [white, black] = await Promise.all([
      getPolicies({ policy_type: 'whitelist', page: 1, page_size: 20 }),
      getPolicies({ policy_type: 'blacklist', page: 1, page_size: 20 })
    ])
    const apply = (res, key) => {
      const p = (res.data || [])[0] || null
      bwPolicy[key] = p ? p.id : null
      bwRules[key] = p ? (p.rules || []).filter(r => r.rule_type === 'software_name') : []
    }
    apply(white, 'whitelist')
    apply(black, 'blacklist')
    const flatten = (key) => bwRules[key].map(r => ({
      policy_id: bwPolicy[key],
      name: r.rule_value
    }))
    whiteList.value = flatten('whitelist')
    blackList.value = flatten('blacklist')
  } catch (error) {
...
const submitUpload = async () => {
  ...
  if (!uploadForm.name || !uploadForm.version) return ElMessage.warning('请填写软件名和版本')
  uploading.value = true
  try {
    const formData = new FormData()
    formData.append('file', uploadForm.file.raw)
    // 后端要求 package_info 为 JSON 字符串表单字段
    formData.append('package_info', JSON.stringify({
      package_name: uploadForm.name,
...
const addPolicy = async (type) => {
  ...
  const typeKey = type === 'white' ? 'whitelist' : 'blacklist'
  const rule = {
  ...
  try {
    if (bwPolicy[typeKey]) {
      if (bwRules[typeKey].some(r => r.rule_value === name)) {
        return ElMessage.warning('该软件已在名单中')
      }
      // 已有基线策略：追加一条规则（整体替换 rules）
      await updatePolicy(bwPolicy[typeKey], { rules: [...bwRules[typeKey], rule] })
    } else {
      // 首次添加：创建该类基线策略
      const res = await createPolicy({
        policy_name: type === 'white' ? '软件白名单基线' : '软件黑名单基线',
        policy_type: typeKey,
        description: '软件中心黑白名单维护（单策略多规则）',
        enabled: true,
        priority: 0,
        target_type: 'all',
        target_ids: [],
      ...
      bwPolicy[typeKey] = res.policy_id
      bwRules[typeKey] = [rule]
    }
    ElMessage.success('已添加')
    if (type === 'white') newWhiteName.value = ''
    else newBlackName.value = ''
    loadPolicies()
  } catch (e) {
    // 错误提示由全局拦截器弹出
  }
...
const removePolicy = async (type, row) => {
  const typeKey = type === 'white' ? 'whitelist' : 'blacklist'
  if (!bwPolicy[typeKey]) return
  try {
    await ElMessageBox.confirm(`确定移除「${row.name}」吗？`, '提示', { type: 'warning' })
    const remaining = bwRules[typeKey].filter(r => r.rule_value !== row.name)
    if (remaining.length === 0) {
      // 规则清空后删除空策略，保持策略列表干净
      await deletePolicy(bwPolicy[typeKey])
      bwPolicy[typeKey] = null
      bwRules[typeKey] = []
    } else {
      await updatePolicy(bwPolicy[typeKey], { rules: remaining })
      bwRules[typeKey] = remaining
    }
