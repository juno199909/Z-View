...
const canCancelTask = (task) => {
  return ['pending', 'running', 'downloading', 'installing'].includes(task.status)
}
...
const cancelTask = async (task) => {
  try {
    ...
    await cancelSoftwareTask(task.id)
    ElMessage.success('Task cancelled')
    loadTasks()
  } catch (error) {
    if (error !== 'cancel') {
      ElMessage.error('Cancel failed: ' + (error.response?.data?.detail || error.message))
    }
  }
}
...
const retryTask = async (task) => {
  try {
    ...
    await retrySoftwareTask(task.id)
    ElMessage.success('Task queued for retry')
    loadTasks()
  } catch (error) {
    if (error !== 'cancel') {
      ElMessage.error('Retry failed: ' + (error.response?.data?.detail || error.message))
    }
  }
}
