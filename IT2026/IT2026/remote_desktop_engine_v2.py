...
class RemoteDesktopSession:
    ...
    def __init__(self, websocket: WebSocket, session_id: str):
        ...
        # P2-06: VDD 修复熔断器（连续失败 ≥3 次进入 FAILURE_STATE，指数退避冷却）
        self._vdd_repair_failures = 0
        self._vdd_circuit_open = False
        self._vdd_circuit_open_at = 0.0
        self._vdd_circuit_cooldown = 600.0  # 初始 10 分钟
        self._VDD_CIRCUIT_BASE_COOLDOWN = 600.0  # 基础冷却 10 分钟
        self._VDD_CIRCUIT_MAX_COOLDOWN = 1800.0  # 最大 30 分钟
        self._VDD_CIRCUIT_THRESHOLD = 3  # 连续失败阈值
    ...
    async def _ensure_persistent_display_substrate(
        ...
        # P2-06: VDD 修复熔断器——连续失败 ≥3 次进入 FAILURE_STATE
        if self._vdd_circuit_open:
            elapsed = now - self._vdd_circuit_open_at
            if elapsed < self._vdd_circuit_cooldown:
                self._log_session_event(
                    "vdd-circuit-breaker",
                    f"OPEN cooldown={self._vdd_circuit_cooldown:.0f}s "
                    f"elapsed={elapsed:.0f}s failures={self._vdd_repair_failures}",
                )
                return {
                    "attempted": False,
                    "recovered": False,
                    "reason": reason,
                    "blocker": "vdd_circuit_breaker_open",
                    "virtual_display_status": status,
                }
            # 冷却结束，允许一次重试（半开状态）
            self._log_session_event(
                "vdd-circuit-breaker",
                f"HALF-OPEN cooldown expired, allowing retry (failures={self._vdd_repair_failures})",
            )
            self._vdd_circuit_open = False
        ...
        # P2-06: VDD 熔断器——跟踪修复成功/失败
        if recovered:
            if self._vdd_repair_failures > 0:
                self._log_session_event(
                    "vdd-circuit-breaker",
                    f"CLOSED: repair succeeded after {self._vdd_repair_failures} failures",
                )
            self._vdd_repair_failures = 0
            self._vdd_circuit_open = False
            self._vdd_circuit_cooldown = self._VDD_CIRCUIT_BASE_COOLDOWN
        else:
            self._vdd_repair_failures += 1
            if self._vdd_repair_failures >= self._VDD_CIRCUIT_THRESHOLD:
                self._vdd_circuit_open = True
                self._vdd_circuit_open_at = time.time()
                # 指数退避：每次触发冷却翻倍，上限 30 分钟
                self._vdd_circuit_cooldown = min(
                    self._vdd_circuit_cooldown * 2,
                    self._VDD_CIRCUIT_MAX_COOLDOWN,
                )
                self._log_session_event(
                    "vdd-circuit-breaker",
                    f"OPEN: {self._vdd_repair_failures} consecutive failures, "
                    f"cooldown={self._vdd_circuit_cooldown:.0f}s",
                )
