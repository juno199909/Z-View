...
真实执行 Windows 防火墙/USB/进程管控操作，并提供启动项/服务/网络连接采集
...
def _parse_netsh_firewall_state(stdout: str) -> Dict[str, str]:
    """解析 netsh advfirewall show allprofiles state 输出（兼容中英文环境）"""
    ...
    for line in (stdout or "").splitlines():
        ...
        if low.endswith("profile settings:") or ("配置文件" in head and ("设置" in s or "配置" in s)):
            current = NETSH_PROFILE_NAMES.get(head, NETSH_PROFILE_NAMES.get(head.rstrip(":：")))
        elif current and ("on" in low or "off" in low or "启用" in s or "关闭" in s):
            enabled = ("on" in low and "off" not in low) or ("启用" in s and "关闭" not in s)
            profiles[current] = "enabled" if enabled else "disabled"
    return profiles


def get_firewall_status() -> Dict[str, Any]:
    """查询防火墙启用状态（Get-NetFirewallProfile 跨语言环境稳定，netsh 兜底）"""
    ps_script = (
        "Get-NetFirewallProfile | "
        "Select-Object Name,Enabled | ConvertTo-Json -Compress"
    )
    r = _run('powershell -NoProfile -Command "' + ps_script + '"', timeout=25)
    profiles: Dict[str, str] = {}
            ...
            if isinstance(data, list):
                for item in data:
                    ...
                    if name in ("domain", "private", "public"):
                        profiles[name] = "enabled" if item.get("Enabled") in (True, "True", 1) else "disabled"
        except (json.JSONDecodeError, TypeError):
            profiles = {}
    if not profiles:
        nr = _run("netsh advfirewall show allprofiles state", timeout=15)
        raw = (nr.get("stdout") or "")[:800]
        profiles = _parse_netsh_firewall_state(raw)
    return {
        "success": bool(profiles) or r["success"],
        "profiles": profiles,
...
# ============================================================
# 行为采集（启动项/服务/网络连接，供安全状态上报与远程运维使用）
# ============================================================
