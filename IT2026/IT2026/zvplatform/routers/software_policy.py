"""
软件策略管理API（1.9.55 自 8082 独立服务迁入主服务 #18 批次B，逻辑逐字保留）
包含黑名单、白名单、强制安装策略
"""
...
def get_db_connection():
    """获取数据库连接（1.9.55 迁移：统一走 zvplatform.db，行为一致）。"""
...
@router.get("/api/v1/policies/logs")
def get_policy_logs(
...
@router.post("/api/v1/policies/logs")
