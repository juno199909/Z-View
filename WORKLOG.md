...
> Newest entry at the bottom (append below). See CLAUDE.md for the ledger rules.
...
## [2026-09-15] 托盘/窗口渲染异常（黑屏家族）统一根因假设：待重启的显示驱动

- 证据链
  ① helper 生命周期日志（无时间戳行 = helper 进程自写）：托盘图标**多次成功
     添加**（23104 等 5+ 对 listening+added），用户双击看到的品牌窗口仍黑屏
     —— 源码级窗口构建（假数据 46 控件）与逻辑验证全部正常 → 排除代码逻辑；
  ② 最后一次 helper（23197）**卡在 listening → native tray icon added 之间**
     = CreateWindowExW 阶段挂起（窗口都建不出来）；
  ③ 会话渲染子系统近期变化：JOB-WUINST-2241-V3（09-15 10:50）安装了
     Broadcom 显示驱动更新 + 系统补丁，reboot_required=1 **至今未重启**；
  ④ 黑屏症状史：1.9.28/29 时代（驱动安装前）已有白屏卡顿/黑窗口——
     但那时同样有更早的 staged 待重启项可能存在，且会话经历多次 RDP 重连。
- 统一根因假设
  会话渲染子系统处于显示驱动 staged/异常状态 → 窗口创建挂起、绘制黑屏、
  交互卡顿（系统级问题，非 UI 代码问题——源码级验证全过）。
- 行动
...
## [2026-09-14] 资产删除二次确认：输入"确认删除"方可执行

- 实现（前端 + 服务端双层校验）
  ① 前端 List.vue：单删/批删确认框改为 ElMessageBox.prompt——弹窗说明
     硬删后果（彻底删除及全部关联数据、在线终端会被心跳重建），要求输入
     "确认删除"（inputPattern 精确匹配），输入错误无法提交；
...
## [2026-09-14] 资产删除改硬删（用户决策）：级联清除全部关联数据 + 审计

- 变更（assets_api.py）
  ① 单删/批量删由软删（deleted_at 标记）改为**硬删除**：资产行与 28 张
     asset_id 关联表（心跳/软件清单/告警/事件/远控会话/设备凭据/令牌/
     升级历史/网络接口/流量质量/USB/安全事件等）一并物理删除；
  ② system_activity_logs 为全局审计不在清除范围，删除动作本身写入
     操作日志（action=delete，含操作人/主机名/删除行数，warning 级）；
  ③ 批量删除逐台审计；删除后若终端仍装有 Agent，下个心跳按新资产自动
     重建（既有的创建新资产分支）。
- 验证（进程内端到端）：造测试资产 → 单删（资产/关联/残留全零 + 审计行
  正确）→ 批删 ✓；测试审计行已清理。
- 语义提示：装着 Agent 的在用终端被删后 30s 内会以全新资产复活——彻底
  移除一台终端仍需先卸载其 Agent 服务。
...
## [2026-09-14] 空壳资产清理 + 终端发现菜单收纳（用户决策）

- 空壳资产清理
  31 台 not_installed 空壳（历史发现扫描遗留，仅 2 台终端真实在线）连同
  关联数据（asset_changes 91 / alerts 1 / batch_operation_results 2 /
  software_task_results 1）事务内硬删。assets 剩 10 台全部 installed。
- 终端发现收纳
  监控菜单移除"终端发现"入口（保留 /discovery 路由与全部代码，直链仍可达）。
...
## [2026-09-14] 日志配置页改路由式页签：彻底消除嵌套样式错位

- 问题反复
  页签内嵌完整页面组件的方案（无论保留还是中和嵌入页外壳）都产生布局错位
  ——两套页面外壳语义叠加，样式补丁越打越歪。

- 终版方案（路由式页签）
  /log/config 变为父路由（ConfigCenter.vue 只渲染页签导航 + <router-view/>），
  三个子路由 retention/notify/threshold 各自渲染原独立页面组件——每页保持
  与控制台其他页面完全一致的标准外观（.zv-page 24px 内边距），零样式补丁。
  页签激活态与路由双向同步。
- 子页保留独立路由路径能力，原 /alert/notify、/alert/thresholds 兼容直链不变。



## [2026-09-14] 日志配置页布局优化：留存配置改为嵌入形态

- 问题：RetentionConfig 按独立页面设计（zv-page 外壳 + 大标题），嵌进
  日志配置页签后双重内边距、标题重复、宽度错位，观感差。
- 修复
  ① RetentionConfig 重构为嵌入形态：根节点去掉 zv-page 外壳，大标题移除
     （页签已表达），说明文字与操作按钮合并为卡片上方工具栏一行；
  ② ConfigCenter 对嵌套 .zv-page 做 :deep 中和（padding:0 / 去限宽），
     并隐藏与页签重复的大标题（保留副标题与操作按钮）——通知配置与
     告警配置两个嵌入页同步受益。



## [2026-09-14] 监控中心配置入口收敛：日志配置统一页（用户决策）

- 变更
  ① 告警阈值 → 更名"告警配置"（路由 /alert/thresholds 与页面标题同步）；
  ② 新增 /log/config 统一配置页（ConfigCenter.vue），三个页签：日志留存 /
     通知配置 / 告警配置——复用现有 RetentionConfig / NotifyConfig /
     ThresholdConfig 组件（lazy）；
  ③ 监控中心菜单收敛为：终端日志、告警中心、日志总览、操作日志、日志配置
     （移除独立的 通知配置/告警阈值 菜单项；其路由保留兼容直链）。
- 未改动：三块配置的功能逻辑、后端接口、原路由路径。



## [2026-09-14] 菜单更名：告警中心→终端日志、事件列表→告警中心（用户决策）

- 更名映射（与页面实际内容对齐）
  ① /alert（终端告警明细流）：菜单/路由标题 告警中心 → **终端日志**（页面内
     标题本就是"终端日志"，告警徽标保留）；
  ② /incidents（终端维度告警聚合与闭环）：菜单/路由标题/页面标题 事件列表 →
     **告警中心**，副标题措辞同步为"告警事件"。
- 未改动：路由路径、组件名、页面内部"事件"术语（incident 本义）。
...
## [2026-09-14] 备份表清理 + 心跳留存收紧至 15 天（用户决策）

- 备份表：backup_20260717_* 共 15 张（14.2 万行，含 13.8 万行旧心跳）已全部
  DROP——用户确认 7/17 手工备份无用。
- 心跳留存：agent_heartbeat 30 天 → **15 天**（DATA_RETENTION_DAYS）。重启后
  worker 启动清理生效：113,764 → 80,259 行（>15 天: 0，区间 8/30~9/14），
  表体量约 25MB。
...
- **验证（15:31 心跳数据）**：Juno 的 `network_interfaces` 表中 **WLAN（Intel Wireless-AC 9560，172.17.40.137，网关 172.17.40.254）`is_primary=1`**；标称 4294 Mbps 的 vgate0 隧道网卡（172.30.242.137）已降为 `is_primary=0`。网络状态页"主网卡"修复生效，路由选择逻辑（1.9.59 资产 IP + 1.9.72 状态页）全部一致。
- **附带说明**：1.9.72 因清除定向名单也下发了 2213/2241，路由优先选卡逻辑对物理机/虚拟机均无害。
- **该会话遗留事项**：① 零拷贝 DXGI→NVENC 被 Intel/NVIDIA 跨适配器拓扑阻断，下一步=WGC 在 GTX 1650 D3D11 设备取帧直送 NVENC（native 桥 ABI 已就绪，CUDA/跨适配器路径待实现）；② 新 WT 根 CA 须经 GPO 重分发后终端才能恢复 HTTPS 严格校验（当前回落 HTTP 8080）；③ 该会话的 WORKLOG 记录写在 worktree（calico-cowbell 已删、big-clavicle 临时），重要结论以本根台账为准。
- **提交**：本条与历史索引一并按"台账即改即提交"规则固化。
...
## [2026-09-23] 工作树分批提交：确认两会话工作全部入库，解除 375 改动风险

- **核查结论**：383 个待处理项中，306 个"M"为行尾/时间戳幻影（git diff 内容差异为 0），68 个"??"为 level-3 未知来源代码副本（含敏感文件，已按用户指示暂停处理），真实未提交内容仅 8 个文件——即本会话的两项修复；Codex 会话的全部工作（UDP Datagram、60FPS、抓屏后端、网络选卡、托盘、安全修复、原生桥接）确认已在其自身提交与 7d504c00 合并中入库。
- **提交清单**：① `8083f277` fix: 解决 7 文件冲突标记（-108 行，HEAD 侧）；② `8bf864c8` fix(frontend): 策略中心开关归一化；③ `18a152f7` chore(docs): 移除项目清理报告。
- **验证**：提交后 `git diff` 内容差异为 0；冲突解决与前端修复的编译/测试/构建此前已全绿。
- **补充（CRLF 幻影 M 清理）**：306 个幻影 M 根因 = `core.autocrlf=true` 的 stat 脏标记，抽样验证 `git add` 后暂存差异为 0，遂对全部受影响文件执行 `git add -u` 刷新索引——**零提交、零内容变更**，工作树从 383 项收敛到 68 项（全部为已暂停的 level-3 副本 ?? 项）。HEAD 保持 `c802360b` 不变。
- **遗留**：① 306 个幻影 M（CRLF 归一化可选清理，暂不动以免产生巨量行尾提交）；② 68 个 level-3 副本待用户查明来历后处理（含 auth_state.json/wt_certs 私钥/.env，勿提交勿外传）；③ 上午审计 5 严重/14 高危中的未修项（权限兜底 auth:self、凭据轮换等）仍待安排。


## [2026-09-23] level-3 未知代码副本隔离处置完成（git 工作树归零）

- **处置**：D:\IT2026\IT2026\IT2026\ 下混入的整份代码副本（90 个顶层项、19223 个文件，今日 13:36–14:03 间出现，来历不明）已整体移入隔离目录 D:\IT2026\backups\level3-copy-quarantine-20260923\（同卷移动可回退，未删除任何文件）。
- **比对依据**：副本 14627 文件与正本一致、405 个为旧版本（抽查 L3 全部早于 L4：09-22 vs 09-23）、4191 个为副本独有运行时文件（日志/缓存/auth_state.json 等，无源码价值）。确认正本（level-4，已全部提交）无任何缺失。
