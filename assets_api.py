...
from zvplatform.routers.software import router as software_records_router  # 1.9.55：软件安装记录（#16 迁入）
...
# 远程桌面/远程会话 WS 路由：已迁至 zvplatform/routers/remote_desktop_ws.py（#16 尾批）
...
app.include_router(software_records_router)  # 1.9.55：软件安装记录（#16 迁入）
...
# ============================================================
# 软件管理端点：已迁至 zvplatform/routers/software_management.py（#18 批次A）
# Agent 软件通道（原反向代理 → 8081）现由本服务直接服务；
# 兼容期 8081 独立进程保留，Agent 的 ZVIEW_SOFTWARE_SERVER_URL 批次C 再切。
# ============================================================
...
from zvplatform.routers.remote_desktop_ws import router as remote_ws_router, bind_remote_ws_helpers  # 1.9.55：WS 路由（#16 尾批）
app.include_router(software_management_router)
app.include_router(software_policy_router)  # 1.9.55：软件策略（#18 批次B）
app.include_router(remote_ws_router)  # 1.9.55：WS 路由（#16 尾批）
...
# WS 基建注入（#16 尾批：websocket 路由迁至 zvplatform/routers/remote_desktop_ws.py）
bind_remote_ws_helpers(
    authenticate_websocket_request=authenticate_websocket_request,
    build_agent_auth_headers=build_agent_auth_headers,
    build_remote_shell_audit_hooks=build_remote_shell_audit_hooks,
    close_browser_websocket=close_browser_websocket,
    close_upstream_websocket=close_upstream_websocket,
    get_asset_agent_target=get_asset_agent_target,
    get_remote_desktop_requester=get_remote_desktop_requester,
    get_remote_desktop_task_name=get_remote_desktop_task_name,
    log_remote_desktop_task_result=log_remote_desktop_task_result,
    relay_agent_to_browser=relay_agent_to_browser,
    relay_browser_to_agent=relay_browser_to_agent,
    send_browser_session_error=send_browser_session_error,
