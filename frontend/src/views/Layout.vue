      ...
      <el-scrollbar class="zv-sidebar-scroll">
        ...
        >
          ...
          <el-sub-menu index="/terminal-menu" class="zv-submenu">
            ...
            <el-menu-item index="/agent/manage" class="zv-menu-item">Agent管理</el-menu-item>
            <el-menu-item index="/patches" class="zv-menu-item">补丁管理</el-menu-item>
            <el-menu-item index="/jobs" class="zv-menu-item">任务中心</el-menu-item>
          ...
          <el-menu-item v-if="isAdmin" index="/system/users" class="zv-menu-item">
            <el-icon><User /></el-icon>
            <template #title>
              <span class="zv-menu-label">用户管理</span>
            </template>
          </el-menu-item>
        </el-menu>
      </el-scrollbar>
...
const titleMap = {
  ...
  '/system/users': { title: '用户管理', icon: 'User' }
}
...
onMounted(async () => {
  ...
  // 获取实时角色（用于用户管理等 admin 菜单可见性）
  try {
    const me = await getCurrentUser()
    if (me?.role) profile.role = me.role
  } catch {}
...
.zv-sidebar-menu {
  ...
  &:not(.el-menu--collapse) {
    // 统一文字左对齐：二级菜单文字与一级菜单图标后的文字对齐
    // （覆盖 Element Plus 写入的内联 padding-left: 40px；20px 缩进 + 24px 图标 + 5px 间距 = 49px）
    :deep(.el-menu .el-menu-item) {
      padding-left: 49px !important;
    }

    // 图标与文字垂直居中，避免不同项基线不一致
    :deep(.el-menu-item),
